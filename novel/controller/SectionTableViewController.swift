...
class SectionTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tSection == nil {
            ...
            // TODO 这个地方的菊花不显示 - -！ 启动加载中菊花
            //            loadingView = ViewUtil.loadingView(viewController.view);
            loadingView = ViewUtil.loadingView(self.tableView);
            
            // 使用异步请求
            Http.post(UrlConstants.SECTION, params: ["code": selectedSection.code], callback: sectionCallback)
        } else {
            // 有缓存, 直接使用
    ...
    func updateLastBookSection() {
        DispatchQueue.main.async {
            self.book.lastSectionCode = self.selectedSection.code;
